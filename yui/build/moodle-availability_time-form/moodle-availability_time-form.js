YUI.add('moodle-availability_time-form', function (Y, NAME) {
...
M.availability_time.form.getNode = function(json) {
    ...
    var fromhour = ('' + json.from / 3600).padStart(2, 0);
    var fromminute = ('' + json.from / 60 % 60).padStart(2, 0);
    var tohour = ('' + json.to / 3600).padStart(2, 0);
    var tominute = ('' + json.to / 60 % 60).padStart(2, 0);
...
M.availability_time.form.fillValue = function(value, node) {
    ...
    value.from = new Date(1970, 0, 1, fromhour, fromminute, 0, 0).getTime() / 1000;
    value.to = new Date(1970, 0, 1, tohour, tominute, 0, 0).getTime() / 1000;
};
